...
?>
            ...
            <form action=supdeladmin.php method=get>
              <table>
                     ...
                     <?php
                            if(isset($_GET["txtedit"])){
                                ...
                                $conn->query("insert into tblledger (flddate,fldcode,fldtype,fldbill,fldbalance,cid,branch) values ('$_GET[txtdate]','Sales No: $_GET[txtdel]','Sales','$_GET[txtamount]','$balance','$_GET[txtsup]','$_GET[txtbranch]')"); 
                                   header("Refresh:0; url=supdeladmin.php");
                                }
                                else{
                            ...
                            if (isset($_GET["txtedit"])){
                                if($_GET["txtedit"]!=""){
                                    echo "<a href=supdeladmin.php><button>Add New</button></a>";
                                }
                            }
                            ?>
                                ...
                                $result=$conn->query("select * from tblsales where branch like '$branch'");
                                while($row=$result->fetch_assoc()){
                                   
                                    echo "<tr><td><a href=supdeladmin.php?txtdel=$row[id]><img src=delete.png width=30></a><a href=supdeladmin.php?txtedit=$row[id]><img src=edit.png width=20></a><a href=# onclick=showme($row[id])>ADD</a></td><td>$row[flddate]</td><td>$row[cid]</td><td>$row[fldcode]</td><td>$row[branch]</td><td>$row[delby]</td><td>$row[fldamount]</td></tr>";
                                }
                               echo "</table>";
...
<script>
 ...
 function showme(myid){
    ...
    document.getElementById("mylink").innerHTML="<iframe src=delmodaladmin.php?txtmyid="+myid+" width=100% FrameBorder=1 height=460px></iframe>";
    }
    function closeme(){
        document.getElementById("mylink").style.display="none";
        window.location="supdeladmin.php";
    }
</script>
