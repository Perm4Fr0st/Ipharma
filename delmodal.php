...
$myproduct="";
$result=$conn->query("select * from delivery where id like '$_SESSION[txtmyid]'");
...
$totalqty="";
  $result=$conn->query("select sum(qty) as total from delprod where delid like '$myid'");
  while($row = $result->fetch_assoc()){
     $totalqty=$row["total"];
  }
  $totalam="";
  $totalam1="";
  $result=$conn->query("select sum(price) as totals from delprod where delid like '$myid'");
  while($row = $result->fetch_assoc()){
     $totalam=$row["totals"];
...
while($row = $result->fetch_assoc()){
    ...
    $stocks=$row["stocks"];
            ...
            $qty="";
             ...
             if (isset($_GET["txteditid"])){
                if ($_GET["txteditid"]!=""){
                    $result=$conn->query("select * from delprod where id=$_GET[txteditid]");
                    while($row=$result->fetch_assoc()){
                        $myproduct=$row["prod"];
                        $qty=$row["qty"];
                        $price=$row["price"];
        ...
        ?>
                                <form method=get action=delmodal.php>
                                <table>
            ...
            if (isset($_GET["txteditid"])){
                if($_GET["txteditid"]!=""){
                    echo "<a href=delmodal.php><button>Add New</button></a>";
                }
            }
            ?>
        <?php
        ...
        if(isset($_GET["txteditid"])){
            if ($_GET["txteditid"]==""){
                ...
                $conn->query("insert into delprod (delid,prod,qty,price,bid) values ('$_GET[txtmyid]','$myproduct','$_GET[txtqty]','$_GET[txtprice]','$_GET[txtbranch]')");
  ...
  $info = array();
  $counter=0;
  $result=$conn->query("select * from delprod where delid like '$idmy' and bid like '$branchh'");
  while($row = $result->fetch_assoc()){
     $counter++;
  ...
  for($a=1;$a<=$counter;$a++){
    ...
    $stocks+=$qty1;
      ...
      $conn->query("insert into prodhis (code,date1,type1,debit,bal,bid) values ('$myids','$date1','$type1','$debit','$stocks','$branchhh')");
                ...
                header("Refresh:0; url=delmodal.php");
            }
            else{
                if (isset($_GET["txtcode1"])){
                    
                $conn->query("update delprod set prod='$_GET[txtcode1]',qty='$_GET[txtqty]',price='$_GET[txtprice]' where id=$_GET[txteditid]");
                header("Refresh:0; url=delmodal.php");
        ...
        if (isset($_GET["txtdel"])){
            $conn->query("delete from delprod where id=$_GET[txtdel]");
            header("Refresh:0; url=delmodal.php");
        }

        echo "<table id=customers>";
        echo "<th width=20%>Delete/Edit</th><th>Product Name</th><th>Quantity</th><th>Price</th>";
        $result=$conn->query("select * from delprod where delid like '$_SESSION[txtmyid]'");
        
        while($row=$result->fetch_assoc()){
           //$desc=str_replace(' ', '%20', $desc);
            echo "<tr><td><a href=delmodal.php?txtdel=$row[id]><img src=delete.png width=40px></a><a href=delmodal.php?txteditid=$row[id]><img src=edit.png width=30px></a></td><td>$row[prod]</td><td>$row[qty]</td><td>$row[price]</td></tr>";
        }
       echo "</table>";
