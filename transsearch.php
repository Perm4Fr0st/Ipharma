...
$result=$conn->query("select * from tbldate where id like '$_GET[txtdate]'");
while($row = $result->fetch_assoc()){
    $idate=$row["id"];
        ...
        while($row=$result->fetch_assoc()){ 
            ...
            echo "<tr><td><a href=trans.php?txtid=$idate&txtcode1=$row[id] target=_parent>$row[code]</td><td>$row[stocks]</td></tr>";
