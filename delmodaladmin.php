...
while($row = $result->fetch_assoc()){
    ...
    $stocks=$row["stocks"];
        ...
        ?>
                                <form method=get action=delmodaladmin.php>
                                <table>
            ...
            if (isset($_GET["txteditid"])){
                if($_GET["txteditid"]!=""){
                    echo "<a href=delmodaladmin.php><button>Add New</button></a>";
                }
            }
            ?>
        <?php
  ...
  for($a=1;$a<=$counter;$a++){
    ...
    $stocks+=$qty1;
      ...
      $conn->query("insert into prodhis (code,date1,type1,debit,bal,bid) values ('$myids','$date1','$type1','$debit','$stocks','$branchhh')");
                ...
                header("Refresh:0; url=delmodaladmin.php");
            }
            else{
                if (isset($_GET["txtcode1"])){
  ...
  $conn->query("delete from supdel where id=$_GET[txtdel]");
        ...
        while($row=$result->fetch_assoc()){
           //$desc=str_replace(' ', '%20', $desc);
            echo "<tr><td><a href=delmodaladmin.php?txtdel=$row[id]><img src=delete.png width=40px></a><a href=delmodaladmin.php?txteditid=$row[id]><img src=edit.png width=30px></a></td><td>$row[prod]</td><td>$row[qty]</td><td>$row[price]</td></tr>";
        }
       echo "</table>";
