<html>
    <body>
        <form method=get action=befwastages.php>
        ...
        if(isset($_GET["txtname"])){
            ...
            header("location:befwastages.php");
        ...
        while($row=$result->fetch_assoc()){
            echo "<tr><td><a href=befwastages.php?txtdelid=$row[id]><input type=button value=x></a><input type=button value='$row[flddate]' id=$row[id] onclick=display($row[id])></td></tr>";
        }
        echo "</table>";
        echo "</td>";
        echo "<td>";
...
<?php
            if(isset($_GET["txtid"])){
                echo "<iframe width=100% height=100% src=wastages.php?txtid=$_GET[txtid]></iframe>";
    ...
    <script>
      ...
      // window.open("beftrans.php?txtid="+myid,"_parent");
       window.location="befwastages.php?txtid="+myid,"_parent";
        }
        function displayy(myidd){
           // alert(myid);
          // iframe.contentDocument.location.href = "beftrans.php?txtid="+myid,"_parent";
      // window.open("beftrans.php?txtid="+myid,"_parent");
     //  window.location="befwasad.php?txtidd="+myidd,"_parent";
        }
