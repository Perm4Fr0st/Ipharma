...
<table>
        <form action=product.php method=get>
            ...
            <tr><td>Branch:</td><td>
                                <select name="txtbranch" id="txtbranch" onchange="shows()">
                                <?php
                                $conn = mysqli_connect($servername, $username, $password, $dbase);
        ...
        <?php
            if (isset($_GET["txtedit"])){
                if($_GET["txtedit"]!=""){
                    echo "<a href=product.php><button>Add New</button></a>";
                }
            }
            ?>
        <?php
    ...
    $codes = $_REQUEST["txtcode"];
    $branchs = $_REQUEST["txtbranch"];
	//die($date);
  
    $check=mysqli_query($conn,"select * from stocks where pid like '$codes' and bid like '$branchs' ");
    $checkrows=mysqli_num_rows($check);

        if($checkrows>0) {
          echo '<script>alert("Product Already Exists in this branch")</script>';
            
         } else { 

                $conn->query("insert into product (code,flddesc,type,capital,interest,status,price) values ('$_GET[txtcode]','$_GET[txtdesc]','$_GET[txttype]','$_GET[txtcapital]','$_GET[txtinterest]','$_GET[txtstatus]','$_GET[txtprice]')");
        ...
        echo "<th width=20%>Delete/Edit/History/Physical Inventory</th><th>Code</th><th>Description</th><th>Type</th><th>Capital</th><th>Interest</th><th>Status</th><th>Price</th><th>Branch</th>";
        $result=$conn->query("select product.id as id,product.code,product.flddesc,product.type,product.capital,product.interest,product.status,product.price,
        stocks.pid,stocks.bid as stocks from product,stocks where product.code=stocks.pid and product.id=stocks.id");
        
        while($row=$result->fetch_assoc()){
            $desc=str_replace(' ', '%20', $desc);
            echo "<tr><td><a href=product.php?txtdel=$row[id]>Delete<img src=delete.png width=40px></a><a href=product.php?txtedit=$row[id]><img src=edit.png width=30px></a><a href=# onclick=showme($row[id]);><img src=history.png width=25px></a><a href=# onclick=showmee($row[id]);><img src=pi.png width=25px></a></td><td>$row[code]</td><td>$row[flddesc]</td><td>$row[type]</td><td>$row[capital]</td><td>$row[interest]</td><td>$row[status]</td><td>$row[price]</td><td>$row[stocks]</td></tr>";
        }
       echo "</table>";
