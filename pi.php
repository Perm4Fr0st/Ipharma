...
<form action=pi.php method=get>
            ...
            <tr><td align=right colspan=2><input type=submit value=<?php echo $mysave2;?>>
                            ...
                            if(isset($_GET["txtedit"])){
                                if ($_GET["txtedit"]==""){
                                    ...
                                    header("Refresh:0; url=pi.php");
                                }
                                else{
                                    if (isset($_GET["txtdate"])){
                                    $conn->query("update pi set date='$_GET[txtdate]',status='$_GET[txtstatus]',bid='$_GET[txtbranch]' where id=$_GET[txtedit]");
                                    header("Refresh:0; url=pi.php");
                                    }
                                }
                            ...
                            if (isset($_GET["txtedit"])){
                                if($_GET["txtedit"]!=""){
                                    echo "<a href=pi.php><button>Add New</button></a>";
                                }
                            }
                            ?>
                                ...
                                $result=$conn->query("select * from pi");
                                while($row=$result->fetch_assoc()){
                                   
                                    echo "<tr><td><a href=pi.php?txtdel=$row[id]><img src=delete.png width=30></a><a href=pi.php?txtedit=$row[id]><img src=edit.png width=20></a></td><td><a href=# onclick=showme($row[id])>Generate</a></td><td>$row[date]</td><td>$row[status]</td><td>$row[bid]</td></tr>";
                                }
                               echo "</table>";
