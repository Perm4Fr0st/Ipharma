<html>
    <body>
        <form method=get action=befwasad.php>
        ...
        while($row=$result->fetch_assoc()){
            echo "<tr><td><a href=befwasad.php?txtdelid=$row[id]><input type=button value=x></a><input type=button value='$row[flddate]' id=$row[id] onclick=display($row[id])></td></tr>";
        }
        echo "</table>";
        echo "</td>";
        echo "<td>";
...
<?php
            if(isset($_GET["txtid"])){
                echo "<iframe width=100% height=100% src=wasad.php?txtid=$_GET[txtid]></iframe>";
    ...
    <script>
      ...
      // window.open("beftrans.php?txtid="+myid,"_parent");
       window.location="befwasad.php?txtid="+myid,"_parent";
        }
